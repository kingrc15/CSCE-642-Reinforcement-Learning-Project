...
if __name__ == "__main__":
    ...
    # idx = C.predict(
    #     MIMICzs[train, :]
    # )  # N-D nearest point search: look for points closest to each centroid
    ...
    ax.legend(
        [
            ...
            "95% LB for best AI policy (MIMIC test set)",
        ],
        loc="lower right",
    )
    ...
    fig, ax = plt.subplots()
    ax.boxplot(
        recqvi[:, [19, 21]], labels=["Clinicians", "AI"]
    )  # some evaluations not done here
    plt.xlim(0.5, 2.5)
    plt.ylim(-100, 100)
    ax.set_ylabel("Estimated policy value")
    ...
    bootql, prog = offpolicy_eval_tdlearning_with_morta_mp(
        qldata3train, physpol, ptid, idx, actionbloctrain, Y90, 0.99, 100
    )  # 100 reps
    ...
    qv = np.floor(
        (a + 100) / (200 / nbins)
    )  # converts Q values to integers btw 0 and nbins
    m = prog[:, 1]  # outcome
    ...
    plt.hist(
        data, edges, facecolor="b", weights=np.ones(len(data)) / len(data), alpha=0.5
    )
    data = np.array(d[d["morta"] == 1]["Qoff"])
    plt.hist(
        data, edges, facecolor="r", weights=np.ones(len(data)) / len(data), alpha=0.5
    )
