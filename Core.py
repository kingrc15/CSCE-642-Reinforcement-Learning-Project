...
class PolicyIteration_with_Q(PolicyIteration):
    def __init__(
        ...
        transitions = np.transpose(
            transitions, (2, 0, 1)
        ).copy()  # Change to Action First (A, S, S)
        ...
        if V is None:
            ...
            try:
                assert V.shape in ((self.S,), (1, self.S)), (
                    "V is not the " "right shape (Bellman operator)."
                )
    ...
    def run(self):
        ...
        while True:
            ...
            if n_different == 0:
                ...
                self.iter > 20 and n_different <= 5
            ):  # This condition was added from the Nature Code
                if self.verbose:
                    print((_MSG_STOP))
                break
            else:
...
class AutoEncoder(nn.Module):
    def __init__(self, input_dim, layers="128-128-32"):
        super().__init__()
        layer_sizes = layers.split("-")

        self.encoder = [
            nn.Dropout(0.2),
            nn.Linear(input_dim, int(layer_sizes[0])),
            nn.ReLU(),
        ]

        for i in range(len(layer_sizes) - 2):
            self.encoder.append(nn.Linear(int(layer_sizes[i]), int(layer_sizes[i + 1])))
            self.encoder.append(nn.ReLU())

        self.encoder.append(nn.Linear(int(layer_sizes[-2]), int(layer_sizes[-1])))
        self.encoder = nn.Sequential(*self.encoder)

        layer_sizes.reverse()

        self.decoder = [nn.Linear(int(layer_sizes[0]), int(layer_sizes[1])), nn.ReLU()]

        for i in range(1, len(layer_sizes) - 1):
            self.decoder.append(nn.Linear(int(layer_sizes[i]), int(layer_sizes[i + 1])))
            self.decoder.append(nn.ReLU())

        self.decoder.append(nn.Linear(int(layer_sizes[-1]), input_dim))
...
if __name__ == "__main__":
    ...
    print(MIMICtable, MIMICtable.shape)
    ...
    use_ae = True
    nr_reps = 50  # nr of repetitions (total nr models) % 500
    nclustering = (
        32  # how many times we do clustering (best solution will be chosen) % 32
    )
    ...
    recqvi = np.full(
        (nr_reps * 2, 30), np.nan
    )  # saves data about each model (1 row per model)
    ...
    idxs = np.full(
        (icustayidlist.shape[0], nr_reps), np.nan
    )  # record state membership test cohort
    ...
    MIMICzs = np.concatenate(
        [
            reformat5[:, colbin] - 0.5,
            zscore(reformat5[:, colnorm], ddof=1),
            zscore(np.log(0.1 + reformat5[:, collog]), ddof=1),
        ],
        axis=1,
    )
    ...
    print(MIMICzs, MIMICzs.shape)

    if use_ae:
        input_data = (MIMICzs - MIMICzs.mean()) / MIMICzs.std()
        model = AutoEncoder(47, layers="128-128-16")
        model = train_ae(model, input_data)
        input_data = torch.from_numpy(input_data).float()
        with torch.no_grad():
            model.eval()
            MIMICzs = model.get_latent(input_data).numpy()
    ...
    for modl in range(nr_reps):  # MAIN LOOP OVER ALL MODELS
        ...
        # list of 1 to 5 (20% of the data in each grp) -- this means that train/test MIMIC split are DIFFERENT in all the 500 models
        crossval = 1
        ...
        ma1 = np.array(
            [
                np.median(reformat5[io == 1, iol]),
                np.median(reformat5[io == 2, iol]),
                np.median(reformat5[io == 3, iol]),
                np.median(reformat5[io == 4, iol]),
                np.median(reformat5[io == 5, iol]),
            ]
        )  # median dose of drug in all bins
        ma2 = np.array(
            [
                np.median(reformat5[vc == 1, vcl]),
                np.median(reformat5[vc == 2, vcl]),
                np.median(reformat5[vc == 3, vcl]),
                np.median(reformat5[vc == 4, vcl]),
                np.median(reformat5[vc == 5, vcl]),
            ]
        )
        ...
        uniqueValuesdose = np.concatenate(
            [ma2Values, ma1Values], axis=1
        )  # median dose of each bin for all 25 actions
        ...
        qldata = np.concatenate(
            [
                blocs.reshape(-1, 1),
                idx.reshape(-1, 1),
                actionbloctrain.reshape(-1, 1),
                Y90.reshape(-1, 1),
                r2,
            ],
            axis=1,
        )  # contains bloc / state / action / outcome&reward
        ...
        for i in range(qldata.shape[0] - 1):
            ...
            if qldata[i + 1, 0] == 1:  # end of trace for this patient
                c = c + 1
                qldata3[c, :] = np.array(
                    [qldata[i, 0] + 1, abss[int(qldata[i, 3])], -1, qldata[i, 4]]
                )
        ...
        sums0a0[
            sums0a0 <= transthres
        ] = 0  # delete rare transitions (those seen less than 5 times = bottom 50%!!)
        ...
        physpol = sums0a0 / np.sum(sums0a0, axis=1).reshape(
            -1, 1
        )  # physicians policy: what action was chosen in each state
        ...
        # delete rare transitions (those seen less than 5 times = bottom 50%!!) IQR = 2-17
        ...
        bootql, bootwis = offpolicy_multiple_eval_010518(
            qldata3, physpol, 0.99, 1, 6, 750
        )
        ...
        recqvi[modl, 6] = mquantiles(bootwis, 0.05, alphap=0.5, betap=0.5)[
            0
        ]  # we want this as high as possible
        ...
        idxs[
            test, modl
        ] = idxtest.ravel()  # important: record state membership of test cohort
        ...
        qldata = np.concatenate(
            [
                bloctestmimic.reshape(-1, 1),
                idxtest.reshape(-1, 1),
                actionbloctest.reshape(-1, 1),
                Y90test.reshape(-1, 1),
                np.zeros((idxtest.size, 1)),
                r2[:, 0].reshape(-1, 1),
                ptidtestmimic.reshape(-1, 1),
            ],
            axis=1,
        )  # contains bloc / state / action / outcome&reward
        # 0 = died in Python, 1 = died in Matlab
        ...
        bootmimictestql, bootmimictestwis = offpolicy_multiple_eval_010518(
            qldata3, physpol, 0.99, 1, 6, 2000
        )

        recqvi[modl, 18] = mquantiles(bootmimictestql, 0.95, alphap=0.5, betap=0.5)[
            0
        ]  # PHYSICIANS' 95% UB
        ...
        recqvi[modl, 23] = mquantiles(bootmimictestwis, 0.05, alphap=0.5, betap=0.5)[
            0
        ]  # AI 95% LB, we want this as high as possible

        if (
            recqvi[modl, 23] > 40
        ):  # saves time if policy is not good on MIMIC test: skips to next model
            print(
                "########################## eICU TEST SET #############################"
            )
        ...
        if (
            recqvi[modl, 23] > 0
        ):  #  & recqvi(modl,14)>0   # if 95% LB is >0 : save the model (otherwise it's pointless)
